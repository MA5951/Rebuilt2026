...
public class SwerveDriveEstimator {
    ...
    private Twist2d getTranslationDelta(SwerveModulePosition[] currentPositions) {
        totalDelta = new Translation2d(); // TODO is better to 0 the values then creat a new one totalDelta = vector
                                          // zero;
        numOfSkiddingModules = 0;

        // if (skidDetector.getNumOfSkiddingModules() >= 2) {
        // totalDelta =
        // totalDelta.plus(calculateModuleDisplysment(lastPositions[skidDetector.getLowestIndex()],
        // currentPositions[skidDetector.getLowestIndex()]));
        // totalDelta =
        // totalDelta.plus(calculateModuleDisplysment(lastPositions[skidDetector.getSecoundLowestIndex()],
        // currentPositions[skidDetector.getSecoundLowestIndex()]));
        ...
        for (int i = 0; i < currentPositions.length; i++) {
            ...
            // if (!skidDetector.getIsSkidding()[i] && numOfSkiddingModules < 2) { // TODO
            // you dont need to check numOfSkiddingModules < 2 its in the else
            // totalDelta = totalDelta.plus(arcDelta);
            // numOfSkiddingModules++; //TODO why you add one? why you dont just use
            // getNumOfSkiddingModules()
            // }
    ...
    public void updateOdometry() {
        ...
        if (collisionDetector.getForceVector() < SKIP_ODOMETRY_Gs
            ...
            for (int i = 0; i < sampleTimestamps.length; i++) {
                SwerveModulePosition[] wheelPositions = new SwerveModulePosition[4]; // TODO its better to do the new
                                                                                     // outsid of the loop
    ...
    private Translation2d calculateModuleDisplysment(SwerveModulePosition lastPosition,
        ...
        if (Math.abs(deltaTheta) < Constants.EPSILON) {
            arcDelta = new Translation2d(deltaDistance, currAngle); // TODO its better to zero the vector instade of
                                                                    // creant a new one
        } else {
            Translation2d v1 = new Translation2d(deltaDistance / deltaTheta,
                    prevAngle.minus(Rotation2d.fromRadians(Math.PI / 2))); // TODO its better to zero the vector instade
                                                                           // of creant a new one
            Translation2d v2 = v1.rotateBy(Rotation2d.fromRadians(deltaTheta));
